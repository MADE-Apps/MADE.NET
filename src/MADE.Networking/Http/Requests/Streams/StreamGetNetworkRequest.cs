...
public sealed class StreamGetNetworkRequest : NetworkRequest
{
    ...
    {
        return (TResponse)await this.GetStreamResponseAsync(cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        return await this.GetStreamResponseAsync(cancellationToken).ConfigureAwait(false);
    }
